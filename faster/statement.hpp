...
namespace faster {
	...
	statement_ptr create_if_statement(
		...
		std::vector<expression<boolean>::ptr> exprs,
		std::vector<statement_ptr> statements
	);

	statement_ptr create_switch_statement(
		std::vector<expression<lvalue>::ptr> decls,
		expression<boolean>::ptr expr,
		std::vector<statement_ptr> statements,
		std::unordered_map<boolean, size_t> cases,
		size_t dflt
	);


	statement_ptr create_while_statement(expression<boolean>::ptr expr, statement_ptr statement);

	statement_ptr create_do_statement(expression<boolean>::ptr expr, statement_ptr statement);

	statement_ptr create_for_statement(
		expression<void>::ptr expr1,
		expression<boolean>::ptr expr2,
