		...
		node_ptr parse_expression_tree_impl(compiler_context& context, tokens_iterator& it, bool allow_comma, bool allow_empty) {
			...
			for (; !is_end_of_expression(*it, allow_comma); ++it) {
				if (it->is_reserved_token()) {
					...
					if (it->is_byte()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_byte(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_boolean()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_boolean(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_int16()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_int16(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_int32()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_int32(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_int64()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_int64(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_uint16()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_uint16(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_uint32()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_uint32(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_uint64()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_uint64(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_double32()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_double32(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_double64()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_double64(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
					else if (it->is_character()) {
						operand_stack.push(std::make_unique<node>(
							context, it->get_character(), std::vector<node_ptr>(), it->get_line_number(), it->get_char_index())
						);
					}
